...
4. `02_lora_finetune.ipynb` loads the base model 4-bit, attaches a LoRA adapter, trains for 1 epoch over the subset, evaluates loss on the held-out split, then **pushes just the adapter** (a few hundred MB) to your own `abdelwb/nemotron-mini-4b-daring-anteater-lora` repo on the Hub.
